// ============================================================
//  Journaling Date — Upload Foto (link personal ?rid=...)
...
if (!RID) {
    ...
    document.getElementById("greetingText").textContent =
        "Halo " + NAMA + "! Foto kenang-kenangan kamu opsional — boleh isi berapa aja, ganti kapan aja sebelum hari-H.";
}
...
async function getCompressedBase64(file) {
    try {
        const dataUrl = await withTimeout(compressImage(file, 1200, 0.8), 20000, "Kompres gambar");
...
if (form) {
    form.addEventListener("submit", async (e) => {
        ...
        // Opsional -- boleh isi berapa aja, minimal 1 (nggak ada gunanya submit kosong).
        const chosenIds = ["photo1", "photo2", "photo3", "photo4"].filter(id => {
            const inp = document.getElementById(id);
            return selectedFiles[id] || (inp && inp.files && inp.files.length);
        });
        if (!chosenIds.length) {
            alert("Pilih minimal 1 foto ya.");
            return;
        }
        ...
        try {
            const response = await fetch(`${SUPABASE_URL}/functions/v1/update-journaling-date-photos`, {
                method: "POST",
            ...
            if (result.status === "success") {
                document.getElementById("formSection").innerHTML =
                    ...
                    '<p style="color:var(--text-secondary);margin-top:.5rem;">Makasih ya, foto kamu udah keupdate. Sampai ketemu di sesinya!</p>' +
                    "</div>";
                lucide.createIcons();
            } else {
